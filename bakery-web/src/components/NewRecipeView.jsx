...
import { createRecipe, fetchMaterials } from '../services/api'

function SortableMaterialRow({ mat, onRemove, onUpdate, searchValue, handleSearchChange, selectMaterial, showDropdown, setShowDropdown, getFilteredMaterials, stageLabels, typeLabels, onFocusReload }) {
...
function NewRecipeView({ onBack, onSuccess, showHeader = true }) {
  const [name, setName] = useState('')
  const [type, setType] = useState('dough')
  const [author, setAuthor] = useState('')
  const [expected_temp, setExpectedTemp] = useState('')
  const [loss_rate, setLossRate] = useState('1.00')
  const [materials, setMaterials] = useState([
    { stage: 'base', material_id: '', percentage: '', note: '', unit: '', sortableId: 'init-0' }
  ])
  ...
  function addMaterial(stage) {
    ...
    setMaterials([
      ...materials,
      { stage, material_id: '', percentage: '', note: '', unit: '', sortableId: newId }
    ])
  }
  ...
  function getFilteredMaterials(index) {
    ...
    if (stage === 'preferment') {
      filtered = allMaterials.filter(m => m.type === 'preparation')
    } else {
      filtered = allMaterials.filter(m => m.type === 'preparation' || m.type === 'ingredient' || m.type === 'dough')
    }
    ...
    <div>
      {showHeader && (
        <div className="page-header">
          ...
          <span className="page-title">新建配方</span>
          <button 
      ...
      <div className="form-section">
        ...
        <div className="form-row">
          ...
          <select value={type} onChange={e => setType(e.target.value)}>
            <option value="dough">面团</option>
            <option value="preparation">半成品</option>
          </select>
        </div>
        <div className="form-row">
          <label>作者</label>
            ...
            <div key={stage}>
              ...
              <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={sortableIds} strategy={verticalListSortingStrategy}>
                  <div className="form-section">
                    {stageMaterials.map(mat => renderMaterialRow(mat))}
                    <div className="add-ingredient-btn" onClick={() => addMaterial(stage)}>
        ...
        <div>
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={materials.map((m, i) => m.sortableId)} strategy={verticalListSortingStrategy}>
              <div className="form-section">
                {materials.map((mat, index) => renderMaterialRow({ ...mat, index }))}
                <div className="add-ingredient-btn" onClick={() => addMaterial('base')}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <line x1="12" y1="5" x2="12" y2="19"/>
                    <line x1="5" y1="12" x2="19" y2="12"/>
                  </svg>
